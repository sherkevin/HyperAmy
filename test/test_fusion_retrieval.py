#!/usr/bin/env python3
"""
融合检索测试 - Amygdala + HippoRAG

测试场景：
1. 级联检索（推荐方案）
2. 并行检索 + 分数融合
3. 对比三种模式的效果

使用 Monte Cristo 数据集
"""

import logging
import sys
import time
from pathlib import Path

# 设置日志
logging.basicConfig(
    level=logging.INFO,
    format='[%(levelname)s] [%(name)s] %(message)s'
)

# 设置环境变量
from llm.config import API_KEY, BASE_URL, DEFAULT_EMBEDDING_MODEL, API_URL_EMBEDDINGS
import os
os.environ["OPENAI_API_KEY"] = API_KEY

from workflow.fusion_retrieval import FusionRetriever

print("=" * 100)
print("融合检索测试：Amygdala + HippoRAG")
print("=" * 100)

# 测试数据（Monte Cristo 场景）
chunks = [
    # Chunk 1 - 早餐场景
    '"I have an excellent appetite," said Albert. "I hope, my dear Count, you have the same." '
    '"I?" said Monte Cristo. "I never eat, or rather, I eat so little that it is not worth '
    'talking about. I have my own peculiar habits."',

    # Chunk 2 - 药丸场景
    'The Count took from his pocket a small case made of hollowed emerald, took out a small '
    'greenish pill, and swallowed it. "This is my food," he said to the guests. "With this, '
    'I feel neither hunger nor fatigue. It is a secret I learned in the East."',

    # Chunk 3 - 花园里的拒绝场景
    '"Will you not take anything?" asked Mercedes. "A peach? Some grapes?" '
    '"I thank you, Madame," replied Monte Cristo with a bow, "but I never eat between meals. '
    'It is a rule I have imposed upon myself to maintain my health."',

    # Chunk 4 - 东方哲学（核心答案）
    '"In the countries of the East, where I have lived," said Monte Cristo to Franz, '
    '"people who eat and drink together are bound by a sacred tie. They become brothers. '
    'Therefore, I never eat or drink in the house of a man whom I wish to kill. '
    'If I shared their bread, I would be forbidden by honor to take my revenge."',

    # Chunk 5 - 情感对峙
    'Mercedes looked at him with terror in her eyes. Her hand trembled as she held the plate. '
    '"You refuse?" she whispered, her voice full of tears. "Is it because you are our enemy? '
    'To refuse to break bread... means you bring death to this house." '
    'She realized then that the man standing before her was not just a visitor, but an avenger '
    'who remembered the past.'
]

# 定义 chunk 类型
chunk_types = {
    0: "早餐场景",
    1: "药丸场景",
    2: "拒绝葡萄干",
    3: "东方哲学（核心答案）",
    4: "情感对峙"
}

query = "Why did the Count strictly refuse the muscatel grapes and any refreshment offered by Madame de Morcerf (Mercedes) during his visit to her house?"

# ========== 初始化融合检索器 ==========
print("\n" + "=" * 100)
print("【初始化】融合检索器")
print("=" * 100)

fusion = FusionRetriever(
    amygdala_save_dir="./test_fusion_amygdala_db",
    hipporag_save_dir="./test_fusion_hipporag_db",
    llm_model_name="DeepSeek-V3.2",
    auto_link_particles=False
)
print("✓ 融合检索器初始化完成")

# ========== 添加数据 ==========
print("\n" + "=" * 100)
print("【添加数据】")
print("=" * 100)

add_start = time.time()
result = fusion.add(chunks)
add_time = time.time() - add_start

print(f"✓ 数据添加完成 ({add_time:.2f}s):")
print(f"  - Amygdala 粒子数: {result['amygdala_count']}")
print(f"  - HippoRAG chunks: {result['hipporag_count']}")
print(f"  - 总 chunks: {result['total_chunks']}")

# ========== 测试 1: 级联检索（推荐）==========
print("\n" + "=" * 100)
print("【测试 1】级联检索（推荐方案）")
print("=" * 100)
print(f"\nQuery: {query}")
print(f"模式: HippoRAG 筛选 Top-20 → Amygdala 精排 Top-5\n")

start_time = time.time()
cascade_results = fusion.retrieve(
    query=query,
    hipporag_top_k=20,
    amygdala_top_k=5,
    mode="cascade"
)
cascade_time = time.time() - start_time

print(f"\n【级联检索结果】（耗时: {cascade_time:.2f}s）")
print(f"检索到 {len(cascade_results)} 个 chunks:\n")

for result in cascade_results:
    # 找到 chunk 类型
    chunk_type = "Unknown"
    for i, chunk in enumerate(chunks):
        if result['text'] == chunk:
            chunk_type = chunk_types[i]
            break

    print(f"  Rank {result['rank']}: {chunk_type}")
    print(f"    - HippoRAG 分数: {result['hipporag_score']:.4f}")
    print(f"    - Amygdala 分数: {result['amygdala_score']:.2f}")
    print()

# ========== 测试 2: 并行检索 + 分数融合 ==========
print("\n" + "=" * 100)
print("【测试 2】并行检索 + 分数融合")
print("=" * 100)
print(f"\nQuery: {query}")
print(f"模式: 并行检索 + 加权融合 (α=0.4, β=0.6)\n")

start_time = time.time()
parallel_results = fusion.retrieve(
    query=query,
    amygdala_top_k=5,
    mode="parallel"
)
parallel_time = time.time() - start_time

print(f"\n【并行检索结果】（耗时: {parallel_time:.2f}s）")
print(f"检索到 {len(parallel_results)} 个 chunks:\n")

for result in parallel_results:
    # 找到 chunk 类型
    chunk_type = "Unknown"
    for i, chunk in enumerate(chunks):
        if result['text'] == chunk:
            chunk_type = chunk_types[i]
            break

    print(f"  Rank {result['rank']}: {chunk_type}")
    print(f"    - HippoRAG 分数: {result['hipporag_score']:.4f}")
    print(f"    - Amygdala 分数: {result['amygdala_score']:.2f}")
    print(f"    - 融合分数: {result['fusion_score']:.4f}")
    print()

# ========== 测试 3: 仅 Amygdala（对比）==========
print("\n" + "=" * 100)
print("【测试 3】仅 Amygdala（对比）")
print("=" * 100)

start_time = time.time()
amygdala_only_results = fusion.retrieve(
    query=query,
    amygdala_top_k=5,
    mode="amygdala_only"
)
amygdala_only_time = time.time() - start_time

print(f"\n【Amygdala 结果】（耗时: {amygdala_only_time:.2f}s）")
for result in amygdala_only_results:
    chunk_type = "Unknown"
    for i, chunk in enumerate(chunks):
        if result['text'] == chunk:
            chunk_type = chunk_types[i]
            break
    print(f"  Rank {result['rank']}: {chunk_type} (得分: {result['score']:.2f})")

# ========== 测试 4: 仅 HippoRAG（对比）==========
print("\n" + "=" * 100)
print("【测试 4】仅 HippoRAG（对比）")
print("=" * 100)

start_time = time.time()
hipporag_only_results = fusion.retrieve(
    query=query,
    amygdala_top_k=5,
    mode="hipporag_only"
)
hipporag_only_time = time.time() - start_time

print(f"\n【HippoRAG 结果】（耗时: {hipporag_only_time:.2f}s）")
for result in hipporag_only_results:
    chunk_type = "Unknown"
    for i, chunk in enumerate(chunks):
        if result['text'] == chunk:
            chunk_type = chunk_types[i]
            break
    print(f"  Rank {result['rank']}: {chunk_type} (得分: {result['score']:.4f})")

# ========== 对比分析 ==========
print("\n" + "=" * 100)
print("【对比分析】")
print("=" * 100)

# 提取排名
def get_rank(results, chunk_idx):
    for result in results:
        for i, chunk in enumerate(chunks):
            if result['text'] == chunk and i == chunk_idx:
                return result['rank']
    return "-"

modes = [
    ("级联检索", cascade_results),
    ("并行检索", parallel_results),
    ("仅 Amygdala", amygdala_only_results),
    ("仅 HippoRAG", hipporag_only_results)
]

print("\n排名对比:")
print(f"{'Mode':<15} {'情感对峙':<15} {'东方哲学':<15} {'拒绝葡萄干':<15}")
print("-" * 65)

for mode_name, results in modes:
    rank_1 = get_rank(results, 4)  # 情感对峙
    rank_2 = get_rank(results, 3)  # 东方哲学
    rank_3 = get_rank(results, 2)  # 拒绝葡萄干

    # 计算平均排名
    ranks = [r for r in [rank_1, rank_2, rank_3] if r != "-"]
    avg_rank = sum(ranks) / len(ranks) if ranks else "-"

    print(f"{mode_name:<15} {str(rank_1):<15} {str(rank_2):<15} {str(rank_3):<15}")

# 性能对比
print("\n性能对比:")
print(f"  {'Mode':<20} {'时间(s)':<15}")
print("-" * 40)
print(f"  {'级联检索':<20} {cascade_time:<15.2f}")
print(f"  {'并行检索':<20} {parallel_time:<15.2f}")
print(f"  {'仅 Amygdala':<20} {amygdala_only_time:<15.2f}")
print(f"  {'仅 HippoRAG':<20} {hipporag_only_time:<15.2f}")

# ========== 总结 ==========
print("\n" + "=" * 100)
print("【总结】")
print("=" * 100)

print("\n✅ 融合检索优势:")
print("  • 级联检索: 兼顾速度和质量（HippoRAG 快速筛选 + Amygdala 深度精排）")
print("  • 并行检索: 保留两个系统的信号，加权融合")
print("  • 灵活配置: 可根据场景选择不同模式")

print("\n📊 推荐使用:")
print("  • 追求质量: 级联检索")
print("  • 追求速度: 仅 HippoRAG")
print("  • 追求稳定: 仅 Amygdala")

print("\n" + "=" * 100)
print("测试完成！")
print("=" * 100)
