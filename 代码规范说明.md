...
- `particle.py`：记忆深度和温度计算（依赖 `emotion.py`）
...
particle = Particle()
result = particle.extract("I'm very happy!", use_specific=True)
...
**允许的依赖关系**：
- ✅ 功能模块 → 基础功能模块（如 `particle` → `llm`）
- ✅ 功能模块 → 外部依赖（如 `poincare` → `chromadb`）
- ✅ 基础功能模块 → 外部依赖（如 `llm` → `requests`）

**禁止的依赖关系**：
- ❌ 功能模块 → 功能模块（如 `particle` → `poincare`）
- ❌ 基础功能模块 → 功能模块（如 `llm` → `particle`）
...
```python
# ✅ 好的接口设计
from particle import Emotion
emotion = Emotion()
vector = emotion.extract(chunk)

# ❌ 不好的接口设计（暴露了太多实现细节）
from particle.emotion import EmotionExtractor, PlutchikEmotionWheel, ...
```
...
1. **功能模块之间的依赖**：
   - `particle` 和 `poincare` 之间**不应该**有依赖关系
